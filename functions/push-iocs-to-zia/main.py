...
def pull_urls(request: Request, _config, logger: Logger) -> Response:
    ...
    logger.info(
        ...
        f"custom_category: {custom_category}, super_category: {super_category}, category_id: {category_id}, action: {action}, urls: {urls}")

    if not all([definition_id, operation_id, url_category_name, category_id, urls]):
        response_body['errors']['description'] = (
            "Missing required credentials: definition_id, operation_id, url_category_name, category_id, urls"
        )
        return Response(body=response_body, code=400)

    logger.info(f"Zscaler pushing IOCs...")

    response_body['urlCategoryConfiguredName'] = url_category_name
    response_body['urlCategoryId'] = category_id
    response_body['urls'] = urls

    zscaler_response = push_iocs_to_zia_with_retry(logger, definition_id, operation_id, url_category_name, category_id,
                                                   action, custom_category, super_category, urls)
...
def push_iocs_to_zia_with_retry(logger, definition_id, operation_id, url_category_name, category_id, action,
                                custom_category, super_category, urls, max_retries=3, backoff_schedule=None):
    ...
    for attempt in range(max_retries + 1):
        response = push_iocs_to_zia(logger, definition_id, operation_id, url_category_name, category_id, action,
                                    custom_category, super_category, urls)

        # Check if we should retry
        if attempt < max_retries and response["status_code"] == http.HTTPStatus.MULTI_STATUS:
            ...
            if resources:
                ...
                if resource_status_code in retryable_codes:
                    ...
                    if resource_status_code == http.HTTPStatus.TOO_MANY_REQUESTS:
                        ...
                        wait_time = get_retry_after_from_headers(logger, resources[0].get("headers", {}))
                    else:
...
def push_iocs_to_zia(logger, definition_id, operation_id, url_category_name, category_id, action, custom_category,
                     super_category, urls):
    """Push IOCs to ZIA"""
    logger.info(
        f"Pushing IOCs to ZIA. definition_id: {definition_id}, operation_id: {operation_id}, url_category_name: {url_category_name},"
        f"custom_category: {custom_category}, super_category: {super_category}, category_id: {category_id}, action: {action}, urls: {urls}"
    )
...
def get_retry_after_from_headers(logger, headers):
    ...
    # Default fallback if header not found or malformed
    logger.info("Retry-After header not found or malformed, using default 5 minutes")
    return 300
